...
for (i in 1:12) {
  ...
  model_cols[[i]] <- c(model=models[[ orders[i] ]]$name, BIC=models[[ orders[i] ]]$BIC)
}
...
PredictSETAR <- function(
  ...
  type=c("naive", "MC", "bootstrap"), Nboot=100, alpha=0.2, 
  ...
  if(single.step) {
    ...
    setar_refit <- EstimSETAR_m_postproc( EstimSETAR_m(x_data, p, d, c, m) )
    ...
    for (t in (nt + 1:horizon)) {
      ...
      x_simulations <- rbind(x_simulations, replicate(Nboot, 
            predictions(x_source, eval.model=setar_refit, tmin=(t - 1))
      ))
  ...
  MSE <- sum((x_eval[1:horizon] - x_pred)^2) / horizon
  # RMSE <- sqrt(MSE)
...
```{r predictionPaths1step, fig.width=11, fig.height=4}
n_ahead <- 1; ne <- length(x_eval)
history <- as.integer(0.01 * nt)
predict_mc <- PredictSETAR(model, x_train, x_eval, single.step=T, type="MC", return.paths=T)
predict_boot <- PredictSETAR(model, x_train, x_eval, single.step=T, type="bootstrap", return.paths=T)

x_mc <- predict_mc$pred
x_boot <- predict_boot$pred

err_mc_low <- predict_mc$se[,1]
err_mc_high <- predict_mc$se[,2]

err_boot_low <- predict_boot$se[,1]
err_boot_high <- predict_boot$se[,2]

x_mc_paths <- predict_mc$paths
x_boot_paths <- predict_boot$paths

x_all <- c(x_mc, x_boot, err_mc_low, err_mc_high, err_boot_low, err_boot_high, x_mc_paths, x_boot_paths)
...
if (length(c) > 1) {
  ...
  plot(x=dat$time[time_range],y=xt[time_range], type="b", lwd=2,
     ylim=plt_range, main=paste(model$name,"Monte Carlo (1-step) sim"), xlab="t", ylab="x[%]")
  # simulations
  matlines(x=matrix(dat$time[(nt - 1) + pred_range], byrow=T), y=x_mc_paths, col=gray(0.6, alpha=0.2), lty=1)
  # x_mc
  lines(x=dat$time[(nt - 1) + pred_range], y=x_mc[pred_range], col="dodgerblue3", lwd=2)
  # conf. intervals
  lines(x=dat$time[(nt - 1) + pred_range], y=err_mc_low[pred_range], col="dodgerblue2", lwd=2, lty="dashed")
  lines(x=dat$time[(nt - 1) + pred_range], y=err_mc_high[pred_range], col="dodgerblue2", lwd=2, lty="dashed")
  # thresholds
  lines(x=c(dat$time[(nt-history)], dat$time[(nt + ne)]), y=c(c[1],c[1]), col="brown3", lty="dashed", lwd=2)
  lines(x=c(dat$time[(nt-history)], dat$time[(nt + ne)]), y=c(c[2],c[2]), col="green3", lty="dashed", lwd=2)
  legend("bottom", horiz=T, legend=c("data", "MC (1-step)", paste0((1 - predict_mc$alpha) * 100,"% conf.interval"), "c1", "c2", "paths"), 
         col=c("black", "dodgerblue3", "dodgerblue2", "brown3", "green3", gray(0.6, alpha=0.2)), 
         lty=c(NA, 1, 2, 2, 2, 1), pch=c(1, NA, NA, NA, NA, NA), lwd=c(2, 2, 2, 2, 2, 1), cex=0.8)
  ...
  lines(x=dat$time[(nt - 1) + pred_range], y=err_mc_high[pred_range], col="dodgerblue2", lwd=2, lty="dashed")  
  # thresholds
  lines(x=c(dat$time[(nt-history)], dat$time[(nt + ne)]), y=c(c,c), col="green3", lty="dashed", lwd=2)
  legend("bottom", horiz=T, legend=c("data", "MC (1-step)", paste0((1 - predict_mc$alpha) * 100,"% conf.interval"), "c", "paths"), 
         col=c("black", "dodgerblue3", "dodgerblue2", "green3", gray(0.6, alpha=0.2)), 
         lty=c(NA, 1, 2, 2, 1), pch=c(1, NA, NA, NA, NA), lwd=c(2, 2, 2, 2, 1), cex=0.8)
}
...
if (length(c) > 1) {
  ...
  plot(x=dat$time[time_range],y=xt[time_range], type="b", lwd=2,
     ylim=plt_range, main=paste(model$name,"Bootstrap (1-step) sim"), xlab="t", ylab="x[%]")
  # simulations
  matlines(x=matrix(dat$time[(nt - 1) + pred_range], byrow=T), y=x_boot_paths, col=gray(0.6, alpha=0.2), lty=1)
  # x_mc
  lines(x=dat$time[(nt - 1) + pred_range], y=x_boot[pred_range], col="dodgerblue3", lwd=2)
  # conf. intervals
  lines(x=dat$time[(nt - 1) + pred_range], y=err_boot_low[pred_range], col="dodgerblue2", lwd=2, lty="dashed")
  lines(x=dat$time[(nt - 1) + pred_range], y=err_boot_high[pred_range], col="dodgerblue2", lwd=2, lty="dashed")
  # thresholds
  lines(x=c(dat$time[(nt-history)], dat$time[(nt + ne)]), y=c(c[1],c[1]), col="brown3", lty="dashed", lwd=2)
  lines(x=c(dat$time[(nt-history)], dat$time[(nt + ne)]), y=c(c[2],c[2]), col="green3", lty="dashed", lwd=2)
  legend("bottom", horiz=T, legend=c("data", "MC (1-step)", paste0((1 - predict_boot$alpha) * 100,"% conf.interval"), "c1", "c2", "paths"), 
         col=c("black", "dodgerblue3", "dodgerblue2", "brown3", "green3", gray(0.6, alpha=0.2)), 
         lty=c(NA, 1, 2, 2, 2, 1), pch=c(1, NA, NA, NA, NA, NA), lwd=c(2, 2, 2, 2, 2, 1), cex=0.8)
  ...
  # thresholds
  lines(x=c(dat$time[(nt-history)], dat$time[(nt + ne)]), y=c(c,c), col="green3", lty="dashed", lwd=2)
  legend("bottom", horiz=T, legend=c("data", "MC (1-step)", paste0((1 - predict_boot$alpha) * 100,"% conf.interval"), "c", "paths"), 
         col=c("black", "dodgerblue3", "dodgerblue2", "green3", gray(0.6, alpha=0.2)), 
         lty=c(NA, 1, 2, 2, 1), pch=c(1, NA, NA, NA, NA), lwd=c(2, 2, 2, 2, 1), cex=0.8)
}
