...
```{r predictPlot1, echo=T, fig.width=10, fig.height=4}
par(mfrow=c(1, 1))
...
for (i in 1:12) {
  predictSETAR_andPlot(
  models[[ orders[i] ]], time=dat$time, x_train=x_train, x_eval=x_eval, 
  pred_type="bootstrap", single.step=T, plot.paths=F)
}
```
...
for (i in 1:12) {
  ...
  predict1_boot <- PredictSETAR(model, x_train, x_eval, single.step=T, type="bootstrap", return.paths=F)
...
We notice that models that quickly diverge have significantly higher $MSE$ than their $\hat\sigma_{\varepsilon}^2$ (`RSS / (n - k)`). 
...
of training data `RSS / (n - k)`, the prediction diverges. In the above table, we mark divergent predictions with `!`.
...
# require(Rcpp)
# 
# cppFunction('double SmoothTransitionC(double x, double c, double gamma, String type) {  
#   if (type == "logistic") {    
#    return 1 / (1 + exp(-gamma * (x - c)));  
#   } 
#   else 
#     if (type == "exponential") {
#       return 1 - exp(-gamma * pow((x - c), 2));  
#     } 
#     else {    
#       return 0;  
#     } 
...
#SmoothTransitionC(x = 1.2, c = 1.5, gamma = 10.0, type = "logistic")
```

#### $m$-Regime Basis

```{r mregBasisDef, echo=T}
...
gradient_CovMatrix <- function(x, p, d, res, Phi, gamma, c, z=x, m=2, type=c("logistic", "exponential")) {
  type <- match.arg(type)
  n <- length(x); k <- max(p, d); times <- (k + 1):n;
  G <- sapply(times, function(t) SmoothTransition(z[t - d], c, gamma, type))
  Phi <- matrix(Phi, nrow=m, byrow=T)
  # transition derivatives
  dG_dgamma <- numeric(); dG_dc <- numeric(); d2G_dgamma2 <- numeric(); 
  d2G_dc2 <- numeric(); d2G_gammadc <- numeric();
  ...
  # skeleton gradient and Hessian
  Y <- sapply(times, function(t) Yt(x, t, p))
  skelGrad <- c(
    Y %*% (1 - G), Y %*% G, 
    as.numeric(sapply(2:m, function(j) (Phi[j,] - Phi[j - 1,]))) %*% Y %*% dG_dgamma,
    as.numeric(sapply(2:m, function(j) (Phi[j,] - Phi[j - 1,]))) %*% Y %*% dG_dc
  )
  zeros <- matrix(0, nrow=m*(p + 1), ncol=m*(p + 1))
  PhiSquareMat <- matrix(c(-Y %*% dG_dgamma, -Y %*% dG_dc), nrow=m*(p + 1), ncol=m)
  skelHess <- c(
    matrix(0, nrow=(p + 1), ncol=m*(p + 1), byrow=T), -Y %*% dG_dgamma, -Y %*% dG_dc,
    
  )
}
```

```{r mEstimSTARDef, echo=T}
...
```{r mEstimSTARPostprocDef, echo=T}
EstimSTAR_m_postproc <- function(model) {
  ...
  c <- round(c, digits=3); gamma <- round(gamma, digits=3) # 3 dec. places seems enough
  model$name <- paste0(ifelse(type == "logistic", "L", "E"),
                "STAR(", p, ",", d, ",", 
                paste(na.omit(c), collapse=','),",", 
                paste(na.omit(gamma), collapse=','), ")")
  return(model)
}
