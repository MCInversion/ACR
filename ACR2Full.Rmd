...
for(i in 1:12) {
  ...
  d <- models[[ orders[i] ]]$d
  c <- models[[ orders[i] ]]$c
  key <- paste0(p, d, round(c, digits=4), sep="/")
...
legend("topleft", legend=c(paste0("SETAR(",models[[orders[1] ]]$p,",",models[[orders[1] ]]$d,",",
                                 round(models[[orders[1] ]]$c, digits=4),")"),
                           paste0("SETAR(",models[[orders[2] ]]$p,",",models[[orders[2] ]]$d,",",
                                 round(models[[orders[2] ]]$c, digits=4),")"),
                           paste0("SETAR(",models[[orders[3] ]]$p,",",models[[orders[3] ]]$d,",",
...
for (i in 1:12) {
  ...
  d <- models[[ orders[i] ]]$d
  c <- models[[ orders[i] ]]$c
  keys[i] <- paste0(p, d, round(c, digits=3), sep=" / ")
...
starParamCovMatrix <- function(x, p, d, res, Phi, gamma, c, z=x, m=2, type=c("logistic", "exponential")) {
  ...
  rssHessians <- lapply(1:(n - k), function(t) 
    2 * (sapply(1:dim_par, function(i) sapply(1:dim_par, function(j) 
      skelGrad[i, t] * skelGrad[j, t])) -  res[t,] * skelHessians[[t]])
  )
  MeanHessian <- Reduce('+', rssHessians) / (n - k)
  rssGrads <- lapply(1:(n - k), function(t) skelGrad[,t] %*% t(skelGrad[,t]))
  MeanInfMatrix <- Reduce('+', rssGrads) / (n - k)
...
# 2 regimes
model <- EstimSTAR_m(xt, p=2, d=2, c=0.1, gamma=10)
Phi = model$PhiParams
res = model$residuals
starParamCovMatrix(xt, p=2, d=2, res, Phi, c=0.1, gamma=10)

# 3 regimes
model <- EstimSTAR_m(xt, p=2, d=2, c=c(0.1,0.2), gamma=c(10,11), m=3)
Phi = model$PhiParams
res = model$residuals
starParamCovMatrix(xt, p=2, d=2, res, Phi, c=c(0.1,0.2), gamma=c(10,11), m=3)
```
