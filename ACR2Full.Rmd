...
for (i in 1:nonLinCount) { # length(nonLinOrders)
  ...
  if ( mean(c(test$p.value) > alpha) > 0.5 ) {
    ...
    cat("remaining nonlinearity rejected SETAR(", p, ",", d, ",", c, ") with mean p-val = ", mean(c(test$p.value) > alpha), "\n")
  }
}
...
EstimSETAR3_postproc <- function(model) {
    ...
    pOrders <- rowSums(matrix(as.numeric((
      sapply(0:(3 * (p + 1) - 1), function(i) ifelse( (i %% 3 != 0 && model$PhiParams[i + 1] != 0), 1, 0) )
    )), nrow=3, byrow=T))
...
EstimSETAR_m_postproc <- function(model) {
    ...
    model$nRegCounts <- nRegCounts
    # names(model$nRegCounts) <- sapply(1:m, function(j) paste0("n", j))
    
    # regime sigma sq:
    regSigmaSq <- rowSums( 
      matrix(as.numeric( 
        sapply(y, function(xi) Indicator_m(xi, c, m)) ), nrow=m) %*% (y - skel)^2 ) / (nRegCounts - k)
    model$regSigmaSq <- regSigmaSq
    ...
    pOrders <- rowSums(matrix(as.numeric((
      sapply(0:(m * (p + 1) - 1), 
             function(i) ifelse( (i %% m != 0 && model$PhiParams[i + 1] != 0), 1, 0) )
    )), nrow=m, byrow=T))
    model$pOrders <- pOrders
    # names(model$pOrders) <- sapply(1:m, function(j) paste0("p", j))
...
for (d in delays) {
  for (p in d:pmax) {
    ...
    orders <- order(sigmas)
    # only the model whose parameter c gives the lowest residual square sum is chosen for postprocessing
    min_sigma_model <- EstimSETAR_m_postproc(pdOmitted[[ orders[1] ]][[1]])
    
    models3[[length(models3) + 1]] <- min_sigma_model
    model3Columns[[length(model3Columns) + 1]] <- c(
      min_sigma_model$pOrders, d, round(min_sigma_model$c, digits=4),
...
names(model3Columns) <- c(
  sapply(1:m, function(j) paste0("p", j)), "d", sapply(1:(m-1), function(j) paste0("c", j)),
...
```{r SETAR3Top3Plot, fig.width=9, fig.height=4}
plotNmax <- 100
par(mfrow=c(1,2))
for (i in 1:min(8, length(orders3))) {
  ...
  name <- paste0("SETAR(",model$p,",",model$d,",",round(c1, digits=3),",",round(c2, digits=3),")");
