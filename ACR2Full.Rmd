...
```{r bdsTesting}
for (i in 1:length(nonLinOrders)) {
  test <- tseries::bds.test(models[[ nonLinOrders[i] ]]$residuals, m = 3)
  p <- models[[ orders[i] ]]$p
  d <- models[[ orders[i] ]]$d
  c <- models[[ orders[i] ]]$c
  if ( mean(c(test$p.value) > alpha) > 0.5 ) {
    cat("possible remaining nonlinearity for SETAR(", p, ",", d, ",", c, ") with mean p-val = ", mean(c(test$p.value) > alpha), "\n")
  } else {
