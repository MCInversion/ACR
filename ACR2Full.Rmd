...
EstimSETAR3_postproc <- function(model) {
    ...
    regSigmaSq <- rowSums( 
      matrix(
        as.numeric((sapply(x, function(xi) Indicator3(xi, c)))), nrow=3)
       %o% (y - skel)^2 ) / (nRegCounts - k)
...
EstimSETAR_m_postproc <- function(model) {
    ...
    regSigmaSq <- rowSums( 
      matrix(
        as.numeric( sapply(x, function(xi) Indicator_m(xi, c, m)) ), nrow=m)
       %o% (y - skel)^2 ) / (nRegCounts - k)
    model$regSigmaSq <- regSigmaSq
    ...
    # count valid p orders
    pOrders <- rowSums(matrix(as.numeric((
      sapply(0:(m * (p + 1) - 1), function(i) ifelse( (i %% m != 0 && model$PhiParams[i + 1] != 0), 1, 0) )
    )), nrow=m, byrow=T))
    model$pOrders <- pOrders
...
str( EstimSETAR_m_postproc( EstimSETAR_m(xt, p=2, d=1, c=c(-0.1, 0.2), m=3) ) )

str( EstimSETAR_m_postproc( EstimSETAR_m(xt, p=2, d=1, c=c(-0.1, 0.1, 0.2), m=4) ) )
