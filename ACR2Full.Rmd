...
cat("data: "); c(xt, x_eval)[nt:(nt + n_ahead)]
```
...
data.frame(cbind(x_out=x_out[nt:(nt + n_ahead)], data=c(xt, x_eval)[nt:(nt + n_ahead)]))
...
for (i in 1:12) {
  model <- models[[ orders[i] ]]
  x_out <- c(xt, rep(0, n_ahead)); 
  
  for (i in 1:n_ahead) {
    x_out[nt + i] <- SETAR_m_singleStep(model, x_out, nt + i)
  }
  plt_range <- 1.3 * c(min(c(x_out, xt, x_eval), na.rm=T), max(c(x_out, xt, x_eval), na.rm=T))
  time_range <- (nt-history):(nt + n_ahead)
  
  c <- model$c
  if (length(c) > 1) {
    plot(x=dat$time[time_range],y=c(xt, x_eval)[time_range], type="b", lwd=2,
...
PredictSETAR(model, x_train, x_eval, n_ahead=n_ahead)$pred
cat(paste0("Monte Carlo (",n_ahead,"-step):\n"))
PredictSETAR(model, x_train, x_eval, n_ahead=n_ahead, type="MC")$pred
cat(paste0("Bootstrap (",n_ahead,"-step):\n"))
PredictSETAR(model, x_train, x_eval, n_ahead=n_ahead, type="bootstrap")$pred
cat("data:\n")
x_eval[1:n_ahead]
