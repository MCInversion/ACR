...
$$
X_t = (`r coef[1]` \pm `r se[1]`) X_{t-1} + (`r coef[2]` \pm `r se[2]`) X_{t-2} + (`r coef[3]` \pm `r se[3]`) X_{t-3} + \varepsilon_t \ , \ \ \hat{\sigma}_{\varepsilon} = `r sigSq`
$$
...
$$
AIC_{SETAR} = \sum_{j=1}^{m}(n_j \log{\hat{\sigma}_{\varepsilon,j}} + 2 (p_j + 1)) \ , \quad BIC_{SETAR} = \sum_{j=1}^{m}(n_j \log{\hat{\sigma}_{\varepsilon,j}} + \log{n_j} (p_j + 1)) \ , \ m = 2
$$
```{r}
# Akaike
...
$n_1$, $n_2$ (regime data counts), $\hat{\sigma}_{\varepsilon,1}$, $\hat{\sigma}_{\varepsilon,2}$ (regime residual variances), and information criteria ($AIC$, $BIC$).
```{r}
...
```
$$
p = `r paste0(1:pmax, collapse=",")` \ , \ d = 1,..., \ p \quad c = `r paste0(round(seq(cmin, cmax, h)[1:3],4), collapse=",")`...`r paste0(round(seq(cmin, cmax, h)[97:100],4), collapse=",")`
$$
...
\end{cases}
$$
$\hat{\sigma}_{\varepsilon}^2 = `r ss` \ , \ \hat{\sigma}_{\varepsilon,1} = `r ss1` \ , \ \hat{\sigma}_{\varepsilon,2} = `r ss2`$
...
$$
p = `r paste0(1:pmax, collapse=",")` \ , \ d = 1,..., \ p \ , \quad c_1, c_2 = `r paste0(round(seq(cmin, cmax, h)[1:3],4), collapse=",")`...`r paste0(round(seq(cmin, cmax, h)[47:50],4), collapse=",")`
$$
...
```{r}
mod <- models[[ orders[3] ]]
p <- mod$p; d <- mod$d; c <- round(mod$c, 2)
phi <- round(mod$PhiParams, 2); se <- round(mod$PhiStErrors,2)
...
$$
X_t = \begin{cases}
`r regStr1` + \varepsilon_t & \text{if } X_{t-`r d`} \leq `r c[1]` \\
`r regStr2` + \varepsilon_t & \text{if } `r c[1]` < X_{t-`r d`} \leq `r c[2]` \\
`r regStr3` + \varepsilon_t & \text{if } X_{t-`r d`} > `r c[2]` \\
\end{cases} \\ \quad \hat{\sigma}_{\varepsilon}^2 = `r ss` 
$$

$\hat{\sigma}_{\varepsilon,1} = `r ss1` \ , \ \hat{\sigma}_{\varepsilon,2} = `r ss2`$
...
`r regStr3` + \varepsilon_t &\quad\text{if } X_{t-`r d`} > `r c[2]`
\end{cases} \\ \quad \hat{\sigma}_{\varepsilon} = `r ss`
$$
...
We notice that models that quickly diverge have a significantly higher $MSE$ than their $\hat{\sigma}_{\varepsilon}^2$ (`RSS / (n - k)`). 
...
\end{matrix}
\\
\quad \hat{\sigma}_{\varepsilon}^2 = `r ss`
$$
...
\end{cases} \\ \quad \hat{\sigma}_{\varepsilon, 1}^2 = `r ss1` \ , \ \hat{\sigma}_{\varepsilon, 2}^2 = `r ss2`
$$
...
\end{cases}
$$
$\hat{\sigma}_{\varepsilon, 1}^2 = `r ss1` \ , \ \hat{\sigma}_{\varepsilon, 2}^2 = `r ss2` \ , \ \hat{\sigma}_{\varepsilon, 3}^2 = `r ss3`$
...
gamma10 = round(nnWghts[[1]][1, 1], 4); gamma11 = round(nnWghts[[1]][2, 1], 4); gamma12 = round(nnWghts[[1]][3, 1], 4);
gamma20 = round(nnWghts[[1]][1, 2], 4); gamma21 = round(nnWghts[[1]][2, 2], 4); gamma22 = round(nnWghts[[1]][3, 2], 4);
...
beta0 = round(nnWghts[[2]][1], 4); beta1 = round(nnWghts[[2]][2], 4); beta2 = round(nnWghts[[2]][3], 4)
```
...
$$
\begin{matrix}
\gamma_1 = (`r gamma10`, `r gamma11`, `r gamma21`)^{\top} \ , \ \gamma_2 = (`r gamma10`, `r gamma12`, `r gamma22`)^{\top} \\
Y_t = (1, X_{t-1}, X_{t-2})^{\top} \ , \ G(x) = 1 /(1 + \text{e}^{-x}) \\
\hat{\sigma}_{\varepsilon}^2 = `r ann_sig2`
\end{matrix}
$$
...
```{r}
mod <- models[[ orders[6] ]]
p <- mod$p; d <- mod$d; c <- round(mod$c, 3)
phi <- round(mod$PhiParams, 2); se <- round(mod$PhiStErrors,2)

Phi <- matrix(phi, nrow=3, byrow=T); Se <- matrix(se, nrow=3, byrow=T);
phi1 <- Phi[1,]; phi2 <- Phi[2,]; phi3 <- Phi[3,]
se1 <- Se[1,]; se2 <- Se[2,]; se3 <- Se[3,]
ss <- round(mod$resSigmaSq, 4); ss1 <- round(mod$regSigmaSq[1], 3); ss2 <- round(mod$regSigmaSq[2], 3); ss3 <- round(mod$regSigmaSq[3], 3)
...
$\hat{\sigma}_{\varepsilon,1} = `r ss1` \ , \ \hat\sigma_{\varepsilon,2} = `r ss2`$
...
```{r}
mod <- models_s3[[ orders_s3[1] ]]
p <- mod$p; d <- mod$d; c <- round(mod$c, 3); gamma <- round(mod$gamma, 3);
phi <- round(mod$PhiParams, 2); se <- round(mod$PhiStErrors,2)
...
```{r}
phi <- round(fitmsm3@Coef,2)
se <- round(fitmsm3@seCoef,2)

ss1 <- round(mean(msmResid(fitmsm3, regime=1)^2), 3)
ss2 <- round(mean(msmResid(fitmsm3, regime=2)^2), 3)
ss3 <- round(mean(msmResid(fitmsm3, regime=3)^2), 3)
...
\end{cases}
$$
$\hat\sigma_{\varepsilon, 1}^2 = `r ss1` \ , \ \hat\sigma_{\varepsilon, 2}^2 = `r ss2` \ , \ \hat\sigma_{\varepsilon, 3}^2 = `r ss3`$
